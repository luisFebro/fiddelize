...
export default async function showNotification(options = {}) {
    const {
        id, // if no id is passed, then notification will show up multiple times even if the same category of message.
        // visual options
    ...
    const config = {
        ...
        actions: [
            // n2
            {
                action: "app-access",
                title: "acessar app",
                icon: "/img/push-notif/star.png", // only works on mobile android - it does not appear in the desktop - the left side icon from the title
            },
        ],
        data,
        vibrate,
    ...
    // MAX ACTIONS - ONLY INFORMATIVE PURPOSE -  If you want to know the number actions that will be displayed by the browser, you can check Notification.maxActions, which is used in the body text in the demo.
    ...
    // END MAX ACTIONS - ONLY INFORMATIVE PURPOSE
