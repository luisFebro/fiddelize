...
export default function InsertNewPassword({ location, match }) {
    ...
    const redirectToLogin = (needRedirectToLogin) =>
        needRedirectToLogin && <Redirect to="/" />;
    ...
    const handlePassword = () => {
        ...
        changePassword(dispatch, bodyPass, userId).then((res) => {
            if (res.status !== 200)
            ...
            setTimeout(
                () =>
                    showSnackbar(
                        dispatch,
                        "Faça o seu login com sua nova senha",
                        7000
                    ),
                5000
            );
    ...
    const showForm = (needRedirectToLogin) =>
        !needRedirectToLogin && (
            <form
                className={!needRedirectToLogin ? "animated zoomIn" : null}
