...
export default function PanelHiddenContent({ data, setRun, run }) {
    ...
    const { adminName, isCustomLoading, isCustom2Loading } = useStoreState(
        (state) => ({
    ...
    useEffect(() => {
        const docsToSkip = 0;
        getStaffBookingListForAdmin(dispatch, _id, docsToSkip).then((res) => {
            if (res.status !== 200)
    ...
    const showStaffAutoCompleteBar = () => (
        <div style={{ marginBottom: "75px" }} className="container-center mt-5">
    ...
    const actions = bookings.map((booking) => ({
        ...
        secondaryHeading: parse(`
                ...
                <br />
                ${
                    typeof booking.bookingDate === "undefined"
                        ? "Sem Agendamento"
                        : calendar(booking.bookingDate)
                }
                <br />
    ...
    const loadMoreDocs = () => {
        ...
        getStaffBookingListForAdmin(dispatch, _id, moreDocsToSkip, true).then(
            (res) => {
                if (res.status !== 200)
                    return showSnackbar(dispatch, res.data.msg, "error");
    ...
    const showMoreButton = () =>
        sizeLoaded === totalDocsSize && sizeLoaded >= limit ? (
            <p className="text-main-container text-center my-3">
                {`${
                    adminName && adminName.cap()
                }, isso é tudo. Não há mais clientes para mostrar.`}
            </p>
        ) : (
            searchTerm.length === 0 &&
            sizeLoaded >= limit && (
                <div className="container-center my-3">
                    <ButtonMulti
                        ...
                        iconFontAwesome={
                            isCustom2Loading ? "" : "fas fa-chevron-circle-down"
                        }
                    />
                </div>
            )
