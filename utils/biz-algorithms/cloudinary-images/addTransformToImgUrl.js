...
function addTransformToImgUrl(lastUrl, paramArray, customParam) {
    ...
    if(!paramArray) { paramArray = ["sizeRect"] }; // default

    const indStart = lastUrl.indexOf("upload/") + 7;
    const startTransform = lastUrl.slice(indStart);
    const lastFoundTransformOrVersion = startTransform.slice(0, startTransform.indexOf("/") + 1)
    const isRawVersion = lastFoundTransformOrVersion.match(/^v\d*/);


    let newParam = ``;
    paramArray && paramArray.forEach((param, ind) => {
        const gotTrailingComma = paramLib[param].slice(-1) === ",";
        if(!gotTrailingComma) throw new Error(`the key ${paramLib[param]} should have a trailing comma in the paramLib`);
    ...
    if(isRawVersion) {
        ...
        const removedLastTransformStr = lastUrl.replace(lastFoundTransformOrVersion, newParam);
