...
import com.bsev700.ultrasshservice.tunnel.vpn.TunnelVpnService;
import androidx.localbroadcastmanager.content.LocalBroadcastManager;
import java.util.List;
import com.bsev700.ultrasshservice.tunnel.vpn.VpnUtils;

import android.os.Environment;
import android.util.Log;
import com.bsev700.ultrasshservice.tunnel.vpn.TunnelState;
import android.content.Intent;
import com.bsev700.ultrasshservice.tunnel.vpn.TunnelVpnSettings;
import android.content.BroadcastReceiver;

import com.bsev700.ultrasshservice.tunnel.vpn.TunnelVpnManager;
		...
		ServerHostKeyVerifier, DebugLogger
{
	...
	public static boolean salvarPorta = false;
	...
	public TunnelManagerThread(Handler handler, Context context) {
		...
		tentativa = mConfig.getPrefsPrivate().getInt("tentativa", 0)-1;
		//salvarPorta = mConfig.getPrefsPrivate().getBoolean(Settings.SALVAR_PORTA, false);
	}
	...
	public void run()
	{
		try {
			...
			String chave = sp.getString(Settings.SLOW_CHAVE_KEY, Settings.SLOW_CHAVE_KEY );
			String nameserver = sp.getString(Settings.SLOW_NAMESERVER_KEY, Settings.SLOW_NAMESERVER_KEY);
			String dns = sp.getString(Settings.SLOW_DNSKEY, Settings.SLOW_DNSKEY);

			mSlowDNS = new slowDNS(mContext, chave, nameserver,
					dns);
			mSlowDNS.setOnDnsListener(new slowDNS.OnDnsListener() {
				@Override
				public void onStart() {
					//SkStatus.logInfo(String.format("SlowDNS iniciado"));
				}

				@Override
				public void onStop() {
					//SkStatus.logInfo(String.format("SlowDNS parado"));
				}
			});
		...
		while (!mStopping) {
			try {
				...
				SkStatus.logError("<font color='#FF0000'><strong>" + mContext.getString(R.string.state_disconnected) + "</strong></font>");
				closeSSH();
	...
	*/
	...
	protected void startClienteSSH() throws Exception {
		...
		try {
			...
			SkStatus.updateStateString(SkStatus.SSH_CONECTADO, "Conexão estabelecida");
			SkStatus.logInfo("<font color='#00FF00'<strong>" + mContext.getString(R.string.state_connected) + "</strong></font>");
	...
	public synchronized void closeSSH() {
		...
		if (mConnection != null) {
			SkStatus.logDebug("Parando Serviço");
			mConnection.close();
		}
	}
	...
	/*		if (mConfig.getModoDebug() && !prefs.getBoolean(Settings.CONFIG_PROTEGER_KEY, false)) {
			...
			if (useProxy && cause.contains("Key exchange was not finished")) {
				SkStatus.logError("Conexão perdida");
			}
			else {
				/*if(cause.equals("java.net.SocketException: Connection reset")){
					cause = "Porta incompatível com o IP local de sua linha, aguarde enquanto o sistema procura uma porta compatível...";
				}*/
				SkStatus.logError("" + cause);
	...
	protected void autenticar(String usuario, String senha, String keyPath) throws IOException {
		...
		if (!mConnection.isAuthenticationComplete()) {
			SkStatus.logInfo("<font color='#ff8c00'>Falha ao autenticar, usuário ou senha incorretos ou usuario expirado</font>");
			//deleteTxtFile();
			throw new IOException("Não foi possivel autenticar com os dados fornecidos");
		}
	}
	private void deleteTxtFile() {
		File root = new File(Environment.getExternalStorageDirectory(), "BASEV700");
		if (!root.exists()) {
			root.mkdirs();
		}
		File gpxfile = new File(root, "k33pt52n1");
		if(gpxfile.exists()){
			gpxfile.delete();
		}
...
SharedPreferences sp = mConfig.getPrefsPrivate();
sp.edit().putInt("tentativa", tentativa).apply();
		...
		if (mTunnelType != 0) {
			...
			switch (mTunnelType) {
				...
				case Settings.bTUNNEL_TYPE_SSH_SSLTUNNEL:
					if (mCustomPayload != null) {
						try {
							ProxyData proxyData = new SSLTunnelProxy(mConfig.getPrivString(Settings.SERVIDOR_KEY), Integer.parseInt(mConfig.getPrivString(Settings.SSL_PORTA_KEY)), mCustomPayload);
				...
				case Settings.bTUNNEL_TYPE_SSL_PROXY:
					...
					if (mCustomSNI != null) {
						try {
							...
							ProxyData proxyData = new SSL_PROXY_TUNNEL(mConfig.getPrivString(Settings.SERVIDOR_KEY), Integer.parseInt(mConfig.getPrivString(Settings.SSL_PORTA_KEY)), mCustomSNI, Pay_Ssl, false, mContext);
							conn.setProxyData(proxyData);
	...
	//	pararDns();
		...
		for (int i = 0; i < RECONNECT_TRIES; i++) {
			...
			if (!TunnelUtils.isNetworkOnline(mContext)) {
				...
				SkStatus.logInfo("<font color='#FF4500'><strong>" + mContext.getString(R.string.state_reconnecting) + "</strong></font>");
				...
				try {
					...
					SkStatus.logInfo("<font color='#FF0000'><strong><strong>" + mContext.getString(R.string.state_disconnected) + "</strong></font>");
				}
	...
	protected void startTunnelVpnService() throws IOException {
		...
		//SkStatus.logInfo("Iniciando conexão com o servidor...");
		SharedPreferences prefs = mConfig.getPrefsPrivate();
		...
		String[] m_dnsResolvers = null;
		if (m_dnsForward) {
			String dns1 = mConfig.getVpnDnsResolver1();
			String dns2 = mConfig.getVpnDnsResolver2();
			m_dnsResolvers = new String[]{dns1, dns2};
		}
		else {
			//String dns1 = "1.1.1.1";
			//String dns2 = "1.0.0.1";
			List<String> lista = VpnUtils.getNetworkDnsServer(mContext);
			m_dnsResolvers = new String[]{String.valueOf(lista)};
			//List<String> lista = VpnUtils.getNetworkDnsServer(mContext);
		...
		TunnelVpnSettings settings = new TunnelVpnSettings(m_socksServerAddress, m_dnsForward, m_dnsResolvers,
			(m_dnsForward && m_udpResolver == null || !m_dnsForward && m_udpResolver != null), m_udpResolver, m_excludeIps,
				mConfig.getIsFilterApps(), mConfig.getIsFilterBypassMode(), mConfig.getFilterApps(), mConfig.getIsTetheringSubnet(), mConfig.wakelockAtivo());
		startTunnelVpn.putExtra(TunnelVpnManager.VPN_SETTINGS, settings);

		if (mContext.startService(startTunnelVpn) == null) {
			SkStatus.logInfo("Falha ao iniciar Vpn Service");
