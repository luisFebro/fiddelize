...
import com.bsev700.ultrasshservice.util.FileUtils;
...
import com.bsev700.ultrasshservice.util.securepreferences.crypto.Cryptor;
import com.bsev700.ultrasshservice.util.securepreferences.model.SecurityConfig;
import com.bsev700.ultrasshservice.util.Cripto;
...
public class ConfigParser
{
	...
	public static boolean convertInputAndSave(InputStream input, Context mContext)
		...
		SharedPreferences.Editor prefsEdit = settings.getPrefsPrivate()
			.edit();
		prefs = mContext.getSharedPreferences("BASEV700GERAL", Context.MODE_PRIVATE);
		...
		try {
			...
			try {
				...
				//String mServidor = prefs.getString(Settings.SERVIDOR_KEY, null);
...
*/
				...
				String dnsResolver = mConfigFile.getProperty(Settings.DNSRESOLVER1_KEY);
				String dnsResolver2 = mConfigFile.getProperty(Settings.DNSRESOLVER2_KEY);
	...
	public static void convertDataToFile(OutputStream fileOut, Context mContext,
		...
		try {
			...
			mConfigFile.setProperty(Settings.DNSRESOLVER1_KEY, settings.getVpnDnsResolver1());
			mConfigFile.setProperty(Settings.DNSRESOLVER2_KEY, settings.getVpnDnsResolver2());
