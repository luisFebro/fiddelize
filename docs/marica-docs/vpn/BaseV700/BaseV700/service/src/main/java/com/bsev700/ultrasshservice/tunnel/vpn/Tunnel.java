...
public class Tunnel
{
			...
			boolean enabledFilter, boolean filterBypassMode, String[] filterApps, boolean enableTethering) throws java.lang.Exception
	{
		...
		//	mHostService.onDiagnosticMessage(routeExcludeMessage.toString());
			...
			tunFd = builder
					...
					.setConfigureIntent(BASEV700Service.getGraphPendingIntent(mHostService.getContext()))
					.establish();
	...
	private boolean routeThroughTunnel(final String socksServerAddress, final String[] dnsResolver, boolean forwardDns, final String udpResolver, final boolean transparentDns) {
		...
		if (forwardDns) {
			...
			try {
				...
				mPdnsd.setOnPdnsdListener(new Pdnsd.OnPdnsdListener() {
					@Override
					public void onStart() {
						//mHostService.onDiagnosticMessage("pdnsd iniciado");
					}

					@Override
					public void onStop() {
						//mHostService.onDiagnosticMessage("pdnsd parado");
						stop();
					}
				});
		...
		mTun2Socks = new Tun2Socks(mHostService.getContext(), tunFd, mMtu,
			...
			mTun2Socks.setOnTun2SocksListener(new Tun2Socks.OnTun2SocksListener() {
				@Override
				public void onStart() {
					//mHostService.onDiagnosticMessage("tun2socks iniciado");
				}

				@Override
				public void onStop() {
					//mHostService.onDiagnosticMessage("tun2socks parado");
					stop();
				}
			});
	...
	private void stopVpn()
	{
		...
		if (tunFd != null)
		{
			try
			{
				mHostService.onDiagnosticMessage("Conexão fechada");
				tunFd.close();
			}
			catch (IOException e) {}
		}
