...
import static com.bsev700.ultrasshservice.tunnel.TunnelManagerThread.salvarPorta;
import static com.bsev700.ultrasshservice.tunnel.TunnelManagerThread.tentativa;
	...
	*/
	...
	public static String parseRotate(String payload) {
		...
		while (match.find()) {
			...
			int split_key;
			if(salvarPorta) {
				lastRotateList.put(i, tentativa);
			}
			if (lastRotateList.containsKey(i)) {
				split_key = lastRotateList.get(i)+1;
				if (split_key >= split.length) {
					SkStatus.logInfo("<strong>Maximo de tentativas atingido! Por favor ligue o modo avião e desligue e tente novamente!</strong>");
					split_key = 0;
				}
			}
			else  {
			...
			//SkStatus.logInfo("Tentando conectar em: <b>"+host+"</b>("+split_key+")");
			SkStatus.logInfo("Tentando conectar em: ****** <b>("+split_key+")</b>");
