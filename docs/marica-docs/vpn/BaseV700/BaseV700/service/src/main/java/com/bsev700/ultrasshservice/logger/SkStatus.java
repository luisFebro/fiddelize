	...
	*/
    ...
    public synchronized static void updateStateString(String state, String msg, int resid, ConnectionStatus level, Intent intent) {
        ...
        if (mLastLevel == ConnectionStatus.LEVEL_CONNECTED &&
            ...
            newLogItem(new LogItem((LogLevel.DEBUG), String.format("Ignoring BASEV700 Status in CONNECTED state (%s->%s): %s", state, level.toString(), msg)));
            return;
        }
        ...
        //newLogItem(new LogItem((LogLevel.DEBUG), String.format("BASEV700 Novo Status (%s->%s): %s",state,level.toString(),msg)));
    }
